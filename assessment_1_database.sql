...
CREATE TABLE `target_audience` (
  `target_audience_id` INT NOT NULL AUTO_INCREMENT,
...
CREATE TABLE `presentation` (
  ...
  `name` VARCHAR(50) NOT NULL DEFAULT '',
  `length_in_hours` DECIMAL(4,2) NOT NULL DEFAULT 0.00,
...
CREATE TABLE `team_has_member` (
  `team_has_member_id` INT NOT NULL AUTO_INCREMENT,
  `team_id` INT NOT NULL,
  `member_id` INT NOT NULL,
  CONSTRAINT `pk_team_has_member` PRIMARY KEY (`team_has_member_id`),
...
CREATE TABLE `team_presentation` (
  ...
  `team_id` INT  NULL, # allowing empty becuase not all teams must have a booking
  `presentation_id` INT  NULL, # allowing empty because not all presentations must have a booking 
...
CREATE TABLE booking (
  ...
  `team_presentation_id` INT NOT NULL, # this is not null on purpose 
  `venue_id` INT NULL,   # allowing because not all venues must have bookings 
...
CREATE TABLE `team_audience` (
  `team_audience_id` INT NOT NULL AUTO_INCREMENT,
  `team_id` INT NOT NULL,
  `target_audience_id` INT NOT NULL,
  CONSTRAINT `pk_team_audience` PRIMARY KEY (`team_audience_id`),
  CONSTRAINT `fk_team_audience_to_team` FOREIGN KEY (`team_id`) REFERENCES `team` (`team_id`) ON DELETE CASCADE,
  CONSTRAINT `fk_team_audience_to_target_audience` FOREIGN KEY (`target_audience_id`)  REFERENCES `target_audience` (`target_audience_id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `presentation_audience` (
  `presentation_audience_id` INT NOT NULL AUTO_INCREMENT,
  `presentation_id` INT NOT NULL,
  `target_audience_id` INT NOT NULL, 
  CONSTRAINT `pk_presentation_audience` PRIMARY KEY (`presentation_audience_id`),
  ...
  CONSTRAINT `fk_presentation_audience_to_target_audience` FOREIGN KEY (`target_audience_id`)  REFERENCES `target_audience` (`target_audience_id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `expert_guide_audience` (
  `expert_guide_audience_id` INT NOT NULL AUTO_INCREMENT,
  `expert_guide_id` INT NOT NULL,
  `target_audience_id` INT NOT NULL,
  CONSTRAINT `pk_expert_guide_audience` PRIMARY KEY (`expert_guide_audience_id`),
  ...
  CONSTRAINT `fk_expert_guide_audience_target` FOREIGN KEY (`target_audience_id`)  REFERENCES `target_audience` (`target_audience_id`) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE `venue_audience` (
  ...
  CONSTRAINT `fk_venue_audience_venue` FOREIGN KEY (`venue_id`) REFERENCES `venue` (`venue_id`)  ON DELETE CASCADE,
